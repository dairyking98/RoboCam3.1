...
No values are pre-populated/defaulted anywhere (including in `SimulationBackend`, which now starts with an empty profile dict) — motion profile values are treated as unknown until actually read from (or set on) a printer. A bundled starting-point preset, `config/motion_profile_presets/ender5_s1_fast_low_vibration.json` (feed 300 XY/20 Z, accel 1500 XY/100 Z, jerk 8 XY/0.4 Z), is a **reasoned but unvalidated** compromise between this printer's own official Klipper config for the same hardware (`max_velocity` 300, `square_corner_velocity` 5.0 — from `Klipper3d/klipper`'s `printer-creality-ender5-s1-2023.cfg`) and general classic-jerk tuning guidance, biased toward the low-vibration side since jerk/accel (not feed rate) dominate settle time on the short well-to-well hops this rig actually makes. It has **not** been validated against real ringing/resonance behavior on the physical unit — treat it as a starting point to refine empirically, not a computed optimum.
...
**Motion Profile Presets**: save/load named slider configurations to `config/motion_profile_presets/<name>.json` (same convention as Experiment Presets). Because motion profiles aren't known to reliably survive a printer power cycle, both **Load** and **(re)connect** actively push gcode rather than passively trusting persistence: Load populates the sliders and immediately Applies (sends M203/M201/M205 + M500) instead of just sitting in the UI; on `SetupPanel.motion_connected`, if a profile was already read/applied/loaded earlier this session it's re-applied immediately (in case the printer's actual state drifted while disconnected), and only falls back to a plain Read when nothing is known yet this session.
