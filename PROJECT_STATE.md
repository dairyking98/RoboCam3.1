...
- **Legacy input path was video**: an older (pre-3.1) version of this pipeline decoded an encoded video file frame-by-frame before tracking. That decode step is now redundant — RoboCam 3.1's raw-burst mode already writes each frame as a `.npy` array directly (see § 3), so video is no longer the only available frame source. Whether the legacy pipeline can consume `.npy` (or the cropped PNGs from post-processing, see § 4) directly instead of round-tripping through an encoded video depends on that pipeline's actual input assumptions — open question until it's shared and reviewed.
...
| Status | Item |
|---|---|
| Investigating (open) | PlayerOne (Mars 662M) raw-burst capture rate is stuck at ~30fps regardless of the camera's advertised 90-120fps. Measured 29.95-29.98fps across 16 well-captures in the 2026-07-01 test dataset (`fps_average` in each well's `*_metadata.json`), fully consistent across 4 separate runs — not a fluke. Camera not connected to verify live as of 2026-07-03; findings below are from reading `robocam/camera.py`, `robocam/experiment.py`, and the bundled SDK docs (`PlayerOne_Camera_SDK_Linux_V3.10.0/python/pyPOACamera.py` — no PDF manual is bundled, this file's comments are the only camera documentation in the repo). No dropped frames found in the sampled data (raw frame indices are fully consecutive 0..N-1 in every well) — this is a throughput/jitter problem, not a capture-stability problem. Two separate classes of cause: **(A) what caps the ceiling** — (1) exposure hardcoded to 20ms at init (`camera.py:169`, `SetExp(..., 20000, False)`) caps max fps to 50 before any overhead, matching the 20ms figure in every captured `camera_meta.json`; (2) `POA_HQI` (SDK config id 27 — "for cameras without DDR (guide camera), reduce frame rate to improve image quality") is never read or set by the code, so it's sitting at whatever the camera's factory default is — the Mars 662M is a guide-camera-class sensor and likely has no DDR buffer, making this a strong candidate; (3) `POA_USB_BANDWIDTH_LIMIT` (SDK config id 28, range 35-100%) is never set; (4) the SDK exposes `GetSensorModeCount`/`GetSensorModeInfo`/`SetSensorMode` for cameras with multiple discrete sensor readout modes, and the code never calls any of them — if the Mars 662M supports more than one mode, a faster one may be unused at index 0. **(B) what causes jitter/instability under whatever the ceiling is** — per-frame timing pulled from real capture data is multi-modal (clusters at ~16-22ms/37-40ms/42-45ms, not a clean floor at any single value), pointing at: (1) `experiment.py:98-103` synchronously `np.save()`s each frame to disk before grabbing the next — no writer thread/queue decouples disk I/O from acquisition, and this is the dominant contributor (the 37-45ms clusters are far larger than any polling-related delay); (2) `get_raw_frame()` (`camera.py:419-424`) polls `ImageReady()` in `time.sleep(0.005)` (5ms) increments before calling `GetImageData()` — this matches the SDK's own documented usage pattern (`GetImageData`'s docstring recommends pre-checking `ImageReady()`), so it's not wrong, but the 5ms interval is our own arbitrary choice and adds up to 5ms of quantization latency per frame, worth tuning down or replacing with a direct blocking `GetImageData(cid, buf, timeout)` call (its internal wait is likely a proper OS-level block in the compiled SDK, tighter than a Python sleep-poll) — a minor fix relative to (1); (3) `get_raw_frame()` allocates a fresh `np.zeros(w*h, ...)` buffer (`camera.py:429`) and then `.copy()`s it (`camera.py:433`) on *every single frame call* — two ~768KB allocations per frame that a reused preallocated buffer would avoid, reducing GC-related jitter for free. **Test plan for next hardware session, ceiling first then jitter:** query `GetSensorModeCount`/`GetSensorModeInfo`; toggle `POA_HQI` off; drop exposure to ~5-8ms (compensate with gain/lighting); check/raise `POA_USB_BANDWIDTH_LIMIT`; then decouple `np.save()` from the acquisition loop (bounded queue + writer thread) and reuse the frame buffer for stability. |
